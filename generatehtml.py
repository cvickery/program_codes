...
def generate_html():
  ...
  for row in cursor.fetchall():
    ...
    values.pop()  # Don’t display institution.
    ...
    cells = ''.join([f'<td>{value}</td>' for value in values]).replace('\'', '’')
    cursor.execute(f"""update registered_programs set html='<tr{class_str}>{cells}</tr>'
